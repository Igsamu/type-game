import { wordsEn as INITIAL_WORDS_EN } from './data.js'
  import { wordsEs as INITIAL_WORDS_ES } from './data.js'
  ...
  const $buttonEn = document.querySelector('#butChangeEn');
  const $buttonEs = document.querySelector('#butChangeEs');
  ...
  initGame(words = INITIAL_WORDS_EN.sort(() => Math.random() - 0.5).slice(0, 50))
  initEvents()
  ...
  $buttonEs.addEventListener('click', function (event) {
    initGame(words = INITIAL_WORDS_ES.sort(() => Math.random() - 0.5).slice(0, 50))
  });
